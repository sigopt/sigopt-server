...
class TestExperiment(UnitTestsBase):
  ...
  def test_experiment_bad_experiment_name_type(self):
    experiment_meta = self.get_experiment_feature("default")
...
class TestSuggestionCreate(UnitTestsBase):
  ...
  def test_wrong_suggestion_id(self):
    experiment_meta = self.get_experiment_feature("default")
    e = self.conn.experiments().create(**experiment_meta)
  ...
  def test_no_suggestion_id(self):
    experiment_meta = self.get_experiment_feature("default")
...
class TestObservationCreate(UnitTestsBase):
  ...
  def test_make_observation_fails_without_assignments_or_id(self):
    experiment_meta = self.get_experiment_feature("default")
    e = self.conn.experiments().create(**experiment_meta)
    with pytest.raises(ValueError) as exception_info:
      self.conn.experiments(e.id).observations().create()
    msg = f"Need to pass in an assignments dictionary or a suggestion id to create an observation"
    assert exception_info.value.args[0] == msg

  def test_make_observation_with_suggestion_id_matches_assignments(self):
    experiment_meta = self.get_experiment_feature("default")
  ...
  def test_make_observation_with_variance(self):
    experiment_meta = self.get_experiment_feature("default")
