...
class TestBestAssignments(UnitTestsBase):
  ...
  def create_observation(self, connection, experiment_id):
    return connection.experiments(experiment_id).observation().create()

  def get_suggestion(self, connection, experiment_id):
    return connection.experiments(experiment_id).suggestions().create()

  def create_local_observations_with_values_in_simplex(self, connection, experiment, on_boundary=False, num=10):
    y1_values = numpy.linspace(0, 1, num=num)
    observations = []

    for y1 in y1_values:  # Create observations according to simplex in lower right quadrant
      y2 = y1 - 1
      suggestion = self.get_suggestion(connection, experiment.id)
  ...
  def assert_observations_are_sorted(observations, experiment):
    default_metric = experiment.constraint_metrics[0] if experiment.is_search else experiment.optimized_metrics[0]
    metric_values = []
    for observation in observations:
      metric_value = next(meval for meval in observation["values"] if meval["name"] == default_metric.name)["value"]
  ...
  def test_failed_observations(self, connection, feature):
    # No observations
    experiment_meta = self.get_experiment_feature(feature)
    experiment = connection.experiments().create(**experiment_meta)
    best_observations = self.fetch_best_assignments(connection, experiment.id)
    assert best_observations.count == 0
    assert best_observations.data == []
  ...
  def test_multimetric(self):
    ...
    experiment = LocalExperimentBuilder(experiment_meta)
    pareto_optimal_observations = self.create_local_observations_with_values_in_simplex(experiment, on_boundary=True)
    non_optimal_observations = self.create_local_observations_with_values_in_simplex(experiment, on_boundary=False)
    ...
    expected_pareto_optimal_observations = [
      o.get_client_observation(experiment) for o in pareto_optimal_observations
    ]
