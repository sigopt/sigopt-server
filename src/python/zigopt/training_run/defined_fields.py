...
from zigopt.training_run.field_types import TRAINING_RUN_API_FIELDS, FieldDetails, FieldType
from zigopt.training_run.model import TrainingRun


class TrainingRunDefinedFieldsExtractor:
  field_details: Sequence[FieldDetails]

  def __init__(
    self, database_service: DatabaseService, query: Query, fields_details: Sequence[FieldDetails] | None = None
  ):
    self.database_service = database_service
    _fields_details = coalesce(fields_details, TRAINING_RUN_API_FIELDS)
    self.id_query = self.database_service.query(aliased(TrainingRun, query.subquery()).id)
    self.fields_details = _fields_details
    self.api_name_to_field_details = {field.api_name: field for field in self.fields_details}

  @generator_to_list
  def _count_primitive_fields(
    self, by_organization: bool
  ) -> Generator[tuple[tuple[str] | tuple[str, str], int], None, None]:
    ...
    if selectables and not by_organization:
      counts = self.database_service.one_or_none(
        self.database_service.query(*[func.count(s) for s in selectables])
  ...
  @generator_to_list
  def _get_user_defined_field_counts(
    self, api_name: str, field_type: FieldType, json_name: str
  ) -> Generator[tuple[tuple[str, str, str], int], None, None]:
    ...
    for result in self.database_service.execute(
  ...
  @generator_to_list
  def _extract_count(self, by_organization: bool) -> Generator[tuple[tuple[str, ...], int], None, None]:
    yield from self._count_primitive_fields(by_organization=by_organization)
    yield from self._count_user_defined_fields()

  @generator_to_list
  def extract_info(self, by_organization: bool = False) -> Generator[DefinedField, None, None]:
    for _field_keys, count in self._extract_count(by_organization=by_organization):
      field_keys = tuple(list_get(_field_keys, i) for i in range(3))
