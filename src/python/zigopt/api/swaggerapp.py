...
class DummyServiceBag(ServiceBag):
  def __init__(self, config_broker):
    ...
    self.database_connection_service = DisabledService(self)  # type: ignore
    self.exception_logger = ExceptionLogger(self)
    self.immediate_email_sender = DisabledService(self)  # type: ignore
    self.logging_service = LoggingService(self)
    self.smtp_email_service = DisabledService(self)  # type: ignore
